...
**OPTIONS STORED IN LOCALS**
	...
	**STARS**
	if "`stars'"==""{
		...
		local starnote "\Figtext{{`notefontsize' Standard errors in parentheses. * p$<.10$, ** p$<0.05$, *** p$<0.01$.}}"
	}
	else if "`stars'"=="robust"{
		local starnote "\Figtext{{`notefontsize' Heteroskedasticity-robust standard errors in parentheses. * p$<.10$, ** p$<0.05$, *** p$<0.01$.}}"
	}
	tokenize "`stars'"
	local stars1 `1'
	local stars2 `2'
	else if "`stars1'"=="cluster"{
		local starnote "\Figtext{{`notefontsize' Standard errors clustered by `2' in parentheses. * * p$<.10$, ** p$<0.05$, *** p$<0.01$.}}"
	}
	else {
